...
def slice_portraits_interactive():
    ...
    prefix = input("Enter filename prefix (default 'image'): ").strip() or "image"
        ...
        with Image.open(image_path) as img:
            ...
            for r_start, r_end in row_ranges:
                for c_start, c_end in col_ranges:
                    image = img.crop((c_start, r_start, c_end, r_end))
                    filename = f"{prefix}-{idx:03}.jpg"
                    image.save(os.path.join(output_dir, filename), "JPEG", quality=95)
                    idx += 1

            print(f"\nSuccess! {idx-1}  images saved to '{output_dir}/'.")
