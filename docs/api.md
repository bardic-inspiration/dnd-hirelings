...
Pure tier of the configurable card element system (config: `public/config/cardUI.yml`).
...
CARD_UI_SCHEMA            // config-editor schema descriptor for cardUI.yml
normalizeCardUIDoc(doc: object): { cards: { [cardName]: CardConfig } }
parseCardUIConfig(ymlText: string): { cards: { [cardName]: CardConfig } }  // yaml.load + normalize
...
`normalizeCardUIDoc` is lenient: malformed sections degrade to empty element
lists, and bar entries accept `[current, max]` lists or `"(current, max)"`
strings; `parseCardUIConfig` throws only on unparseable YAML.
...
### `useCardUIConfig(cardName: string)` → `CardConfig`

Returns one card's element assignments (`{ medallion, boxes, bars, fields,
values, slots }`) from the live cardUI config document — the deployed
`public/config/cardUI.yml` merged with any Configuration Modal overlay, read
from `ConfigContext` and normalized via `normalizeCardUIDoc`. Because the
...
the card UI config against a shared resolution `context`
(`{ agent, dyn, attributes }`); see `src/logic/cardUI.js` for resolution.
