...
`advanceTime` records every contribution to (sub)task progress in `state.eventLog`, one **game day** at a time (a step-forward of `timeStep` days loops the internal single-day `advanceDay`): per day, one `work_contribution` entry per **(agent, condition)**, one `task_complete` entry per task that finishes that day, and one `'tick'` boundary entry sealing the day's batch. The **ordering contract** is `work* → task_complete* → tick`; the tick entry appends on every day (even a no-op one) and records `data: { stepMins, wagesTotal, wages }` — the one-day step size and that day's exact wage payments — so rollback stays correct even if `timeStep` or agent rates change later. `task_complete` entries record the task's tags, `results`, and the exact `spawnedAgentIds` / `unassignedAgentIds` the completion produced. The log is the authoritative, in-state record — a browser SPA can't stream-append to a disk file, so the log lives in state (persisted to localStorage with everything else) and is exported to / imported from CSV on demand via `src/logic/eventLog.js` (`saveEventLogToFile` / `loadEventLogFromFile`, reusing the shared `src/logic/download.js` helper). Logging is configured by `public/config/rollback.yml` (`log.enabled`, `log.maxRows` FIFO cap).
...
setInterval (tick)                        Step-back button
     │  advanceTime(state, configs)            │  rollbackTick × session.stepBack
     │  (loops advanceDay × timeStep)          │  (one day per call)
